...
- RStudio
- visualization
description: The penultimate chapter as TDTVOD plunges headlong into December
---
...
```rdf3 <- filter(df0, Name != "test car park") %>%
    group_by(Name, LastUpdate, DateUploaded) %>%
    filter(n() > 1)

df3```
```rconsole## Source: local data frame [0 x 12]
...
```rdf3 <- select(df0, Name, DateUploaded) %>%
...
```rdf2 <- select(df0, Name, DateUploaded) %>%
    filter(Name != "test car park") %>%
...
```rdf4 <- select(df0, LastUpdate, Capacity, Occupancy, Percentage) %>%
