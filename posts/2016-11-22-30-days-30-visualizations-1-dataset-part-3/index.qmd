...
- RStudio
- visualization
description: The third riveting instalment of TDTVOD
---
...
```rtype <- vector("character", nrow(df3))
...
```rdf4 <- filter(df3, Type == "Overfilled" | Type == "Negative") %>%
...
```rdf4 <- filter(df3, Occupancy < -10000)
...
##                   26                    0                    0```
```rrange(df4$Occupancy)```
```rconsole## [1] -14787 -14638```
```rtime_length(max(as.double(df4$LastUpdate)) - min(as.double(df4$LastUpdate)),
            unit = "hour")```
```rconsole## [1] 0.02361111```
...
```rp <- ggplot(df4, aes(x = LastUpdate, y = Occupancy)) +
    geom_line() +
...
```rdf2 <- select(df, Name, LastUpdate, Occupancy) %>%
...
df4 <- rbind(df2, df3)```
<p>Now let’s plot!</p>
```rp <- ggplot(df4, aes(x = Time, y = newOcc)) +
