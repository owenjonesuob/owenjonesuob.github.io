...
- RStudio
- visualization
description: The epic finale of a month-long journey
---
...
```rdf1 <- select(df0, Name, LastUpdate, DateUploaded,
...
```rdf2 <- select(df1, Name, Percentage, LastUpdate) %>%
...
```rp <- ggplot(data = df2, aes(Time, y = Day)) +
...
```rdf2 <- select(df1, Name, LastUpdate, Percentage) %>%
    filter(Name != "test car park") %>%
