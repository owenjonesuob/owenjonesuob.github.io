...
- RStudio
- visualization
description: TDTVOD Goes Fourth
---
...
```rtimerange <- interval(ymd_hms("2015-01-05 10:00:00"),
...
df2 <- select(df, Name, LastUpdate, Occupancy) %>%
    ...
    arrange(LastUpdate) %>%
    mutate(newOcc1 = ifelse(Occupancy < -10000, Occupancy + k, Occupancy))```
...
```rp <- ggplot(df2, aes(x = LastUpdate, y = newOcc)) +
...
```rdf2 <- select(df, Name, LastUpdate) %>%
...
```rp <- ggplot(df2[-c(1), ], aes(x = count, y = (metacount + 0.5))) +
...
```rdf3 <- filter(df, Name != "test car park") %>%
    group_by(Name, LastUpdate) %>%
    filter(n() > 1)
...
```rp <- ggplot(df3, aes(x = LastUpdate, y = "")) +
...
```r# Read in the original dataframe again
...
```rp + scale_y_log10()```
<img class="alignnone size-full wp-image-674" src="day20log.jpeg" />
