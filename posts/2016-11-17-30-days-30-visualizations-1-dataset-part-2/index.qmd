...
- R
- visualization
description: The second instalment of an ongoing dataviz project
---
...
```
```rconsole##  [1] 96 12 86 94 96 96 96 96 96 96 97 95 96 96 96 96 96 96 96 96 96 98 95
## [24] 95```
...
```rsummary(df$Status)```
```rconsole##          Emptying  Filling   Static 
##     1333   474595   404339   647960```
...
```rsummary(df[df$Name == "test car park", ]$Status)```
```rconsole##          Emptying  Filling   Static 
##     1333        0        0        0```
...
```rconsole## Source: local data frame [6 x 5]
## Groups: Name [1]
## 
...
## 6 Avon Street CP   Fri 0.3864523 0.2756433 0.3379044```
...
df3 <- melt(df2)
```
```rconsole## Using Name, Day as id variables```
```rhead(df3)```>
```rconsole##             Name   Day variable     value
...
## 6 Avon Street CP   Fri  Filling 0.3864523```
...
```rsummary(df[df$Name == "Podium CP",]$Status)```
```rconsole##          Emptying  Filling   Static 
##        0        0        0   191022```
...
str(df)```
```rconsole## 'data.frame':    1528227 obs. of  6 variables:
...
##  $ Status    : Factor w/ 4 levels "","Emptying",..: 4 3 4 4 4 4 3 4 4 4 ...```
<p>That’s more like it. Let’s write that to a file.</p>
```rwrite.csv(df, "df.csv", row.names = FALSE)```
<p>Now let’s load it in again, just to make sure:</p>
```rdf <- read.csv("C:/Users/Owen/Documents/Coding/Parking/df.csv")
...
```rdf2 <- df %>%
    ...
    group_by(Name, Week) %>%
    summarize(meanP = mean(meanP))```
<p>Now let’s have another look at the maximum values per car park:</p>
```rmaxP <- top_n(df2, n = 1)```
```rconsole## Selecting by meanP```
```rmaxP</span>```
```rconsole## Source: local data frame [8 x 3]
## Groups: Name [8]
## 
##                   Name  Week    meanP
##                 <fctr> <dbl>    <dbl>
...
## 8    SouthGate Rail CP    46 65.25588```
...
ggplot(df3, aes(x = ymd_hms(LastUpdate), y = Percentage)) +
    ...
    labs(x = "Time of record", y = "Percentage occupancy")```
<img src="day10.jpeg" />
